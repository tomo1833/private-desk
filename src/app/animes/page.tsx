...
const AnimeListPage = () => {
  ...
  if (!animes) return <div className="text-center p-4">読み込み中...</div>;

  return (
    <div className="space-y-4 page-wrap">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-white">アニメ記録一覧</h1>
        <Link href="/animes/new" className="btn btn-primary text-center">
          新規作成
        </Link>
      </div>
      ...
      {animes.length === 0 && (
        <div className="text-center text-gray-500 dark:text-gray-400 py-12">
          <p className="text-lg mb-4">まだアニメ記録がありません</p>
          <Link href="/animes/new" className="btn btn-primary inline-block">
            最初の記録を作成
          </Link>
        </div>
      )}
