...
const NarouCards: React.FC<Props> = ({ narous, viewMode = 'card', onDelete }) => {
    ...
    return (
      <div className="table-container">
        <table className="table-basic">
          <thead>
            <tr>
              <th className="table-header w-1/3">作品タイトル</th>
              <th className="table-header">感想・あらすじ</th>
          ...
          <tbody>
            {narous.map((narou) => (
              <tr key={narou.id} className="table-row">
                <td className="table-cell font-semibold text-slate-100">
                  <button
                    onClick={() => router.push(`/narous/${narou.id}`)}
                    className="hover:underline text-left text-purple-400 hover:text-purple-300 font-medium"
                  >
                    📖 {narou.title}
                  </button>
                </td>
                <td className="table-cell text-xs text-slate-300">
                  <p className="line-clamp-2">{narou.content}</p>
                </td>
                <td className="table-cell text-xs font-mono text-slate-400">
                  {narou.created_at ? new Date(narou.created_at).toLocaleDateString('ja-JP') : '-'}
                </td>
        ...
        >
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="w-7 h-7 rounded-lg bg-purple-500/20 text-purple-400 flex items-center justify-center text-sm shrink-0">
                📖
              </span>
              <h3
                onClick={() => router.push(`/narous/${narou.id}`)}
                className="font-bold text-base sm:text-lg text-slate-100 group-hover:text-purple-300 transition-colors cursor-pointer truncate flex-1"
              >
                {narou.title}
              </h3>
            </div>
