...
const AnimeCards: React.FC<Props> = ({ animes, viewMode = 'card', onDelete }) => {
    ...
    return (
      <div className="table-container">
        <table className="table-basic">
          ...
          <tbody>
            {animes.map((anime) => (
              <tr key={anime.id} className="table-row">
                <td className="table-cell font-semibold text-slate-100">
                  <button
                    onClick={() => router.push(`/animes/${anime.id}`)}
                    className="hover:underline text-left text-rose-400 hover:text-rose-300 font-medium"
                  >
                    🎬 {anime.title}
                  </button>
                </td>
                <td className="table-cell text-xs text-slate-300">
                  <p className="line-clamp-2">{anime.content}</p>
                </td>
                <td className="table-cell text-xs font-mono text-slate-400">
                  {anime.created_at ? new Date(anime.created_at).toLocaleDateString('ja-JP') : '-'}
                </td>
        ...
        >
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="w-7 h-7 rounded-lg bg-rose-500/20 text-rose-400 flex items-center justify-center text-sm shrink-0">
                🎬
              </span>
              <h3
                onClick={() => router.push(`/animes/${anime.id}`)}
                className="font-bold text-base sm:text-lg text-slate-100 group-hover:text-rose-300 transition-colors cursor-pointer truncate flex-1"
              >
                {anime.title}
              </h3>
            </div>
