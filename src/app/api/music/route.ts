...
export async function GET(request: Request) {
  ...
  try {
    const sql = limit
      ? 'SELECT * FROM music ORDER BY display_order ASC, created_at DESC, id DESC LIMIT ?'
      : 'SELECT * FROM music ORDER BY display_order ASC, created_at DESC, id DESC';
    const results = limit
      ? await runSelect<Music>(sql, [Number(limit)])
      : await runSelect<Music>(sql);
...
export async function POST(req: Request) {
  try {
    ...
    await runExecute('INSERT INTO music (title, content, display_order) VALUES (?, ?, ?)', [
      title,
      content,
      displayOrder,
    ]);
