...
export async function GET(request: Request) {
  ...
  try {
    const sql = limit
      ? 'SELECT * FROM narou ORDER BY display_order ASC, created_at DESC, id DESC LIMIT ?'
      : 'SELECT * FROM narou ORDER BY display_order ASC, created_at DESC, id DESC';
    const results = limit
      ? await runSelect<Narou>(sql, [Number(limit)])
      : await runSelect<Narou>(sql);
...
export async function POST(req: Request) {
  try {
    ...
    await runExecute('INSERT INTO narou (title, content, display_order) VALUES (?, ?, ?)', [
      title,
      content,
      displayOrder,
    ]);
