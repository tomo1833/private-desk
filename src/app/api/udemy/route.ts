...
export async function GET(request: Request) {
  ...
  try {
    const sql = limit
      ? 'SELECT * FROM udemy ORDER BY display_order ASC, created_at DESC, id DESC LIMIT ?'
      : 'SELECT * FROM udemy ORDER BY display_order ASC, created_at DESC, id DESC';
    const results = limit
      ? await runSelect<Udemy>(sql, [Number(limit)])
      : await runSelect<Udemy>(sql);
...
export async function POST(req: Request) {
  try {
    ...
    await runExecute('INSERT INTO udemy (title, content, display_order) VALUES (?, ?, ?)', [
      title,
      content,
      displayOrder,
    ]);
