...
export async function GET(request: Request) {
  ...
  try {
    const sql = limit
      ? 'SELECT * FROM wiki ORDER BY display_order ASC, created_at DESC, id DESC LIMIT ?'
      : 'SELECT * FROM wiki ORDER BY display_order ASC, created_at DESC, id DESC';
    const results = limit
...
export async function POST(req: Request) {
  try {
    ...
    await runExecute('INSERT INTO wiki (title, content, display_order) VALUES (?, ?, ?)', [
      title,
      content,
      displayOrder,
    ]);
