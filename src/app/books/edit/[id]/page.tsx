  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setDisplayOrder(book.display_order ?? 0);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6 page-wrap max-w-2xl mx-auto">
      <div className="flex justify-between items-center">
        ...
        <Link href={`/books/${id}`} className="btn btn-secondary text-xs">
          ← 詳細へ戻る
        </Link>
      </div>
      ...
      <form onSubmit={handleUpdate} className="card-form space-y-4 shadow-2xl border border-indigo-500/30">
        ...
        <div className="flex justify-between items-center pt-4 border-t border-slate-700/80">
          <Link href={`/books/${id}`} className="btn btn-secondary text-xs px-3.5 py-1.5">
            キャンセル
          </Link>
