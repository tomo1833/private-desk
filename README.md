...
取り込んだ記事を Excel ファイルへ書き出す場合は、次のコマンドを実行してください。

```bash
npx ts-node scripts/export-blogger.ts
```

`blogger_list.xlsx` が生成され、HTML タグを除いた本文が保存されます。

ブログ一覧ページの「Bloggerエクスポート」ボタンからも同じファイルをダウンロードできます。
