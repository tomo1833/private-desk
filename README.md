...
- 目的：業務や日常を効率化するための **“マイ・パーソナル環境”**
- 配布・公開：GitHubで「参考になれば」レベルのソース公開はOK。ユーザーサポートや責任は負わない

## 🛡 それでも気をつけたい2つの点

1. **ライセンス表記（念のため）**  
   もしGitHubでソースを公開している場合は、以下のようなシンプルなライセンスを明記しておくと安心です。
   - MIT License：自由度が高く、「使うのも自由／改造も自由／責任は自己責任」が明記される
   - CC BY-NC：コンテンツ（例：Wiki記事、ブログなど）に適用して、「商用利用不可＋クレジット表記義務」のような制限も可能
2. **利用目的の明記**
   本ツールは Raspberry Pi などローカル環境での個人利用を前提に設計されています。
   インターネット公開や多人数での利用は想定していません。
### 主な機能
